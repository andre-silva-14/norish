...
vi.mock('@tanstack/query-persist-client-core', () => ({
  persistQueryClientRestore: (options: unknown) =>
    mockPersistQueryClientRestore(options),
  persistQueryClientSubscribe: (options: unknown) =>
    mockPersistQueryClientSubscribe(options),
}));
...
describe('createPersistedQueryClient', () => {
  ...
  it('restores and subscribes with the expected persistence settings', async () => {
    ...
    expect(
      shouldDehydrateQuery?.({ state: { status: 'success' } }),
    ).toBe(true);
